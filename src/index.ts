...
🔍 DISCOVERY FUNCTIONS - Available inside your code for self-service tool discovery:
  • listAvailableTools(options?) - List all MCP tools with filtering
    Options: { filter?: string, server?: string, includeSchema?: boolean }
  • searchTools(query, limit?) - Search by capability (e.g., "analyze code")
  • getToolSchema(toolName) - Get full parameter schema for a tool
  • callMCPTool(toolName, params) - Call an MCP tool

PROACTIVE WORKFLOW: Discover → Inspect → Execute in a single code block!

Example - Discover and use tools in one execution:
  const tools = await searchTools("analyze code");
  const schema = await getToolSchema(tools.results[0].name);
  const result = await callMCPTool(tools.results[0].name, {...});
